...
class _ArabicLetterQuizScreenState extends State<ArabicLetterQuizScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        title: const Text('كويز الحروف للأطفال', style: TextStyle(fontSize: 24)),
        centerTitle: true,
        ...
        child: Padding(
          ...
          child: finished
              ? Column(
                  ...
                  children: [
                    const Text("النتيجة النهائية:", style: TextStyle(fontSize: 26, fontWeight: FontWeight.bold)),
                    const SizedBox(height: 30),
                    Text("${((correctAnswers / questions.length) * 100).round()}%", style: const TextStyle(fontSize: 50, color: Colors.green)),
                  ],
                )
  ...
  Widget _buildQuestionWidget(Map<String, dynamic> q) {
    switch (q["type"]) {
      ...
      case "audioMatch":
        return Column(
          children: [
            const Text("🎧 استمع واختر الحرف الذي سمعته", style: TextStyle(fontSize: 24)),
            ...
            Wrap(
              ...
              children: q["options"].map<Widget>((opt) {
                return ElevatedButton(
                  ...
                  child: Text(opt, style: const TextStyle(fontSize: 28, fontWeight: FontWeight.bold, color: Colors.white)),
                );
              }).toList(),
            ),
          ],
